...
if(!isset($_SESSION['email']) & empty($_SESSION['email'])){
	...
	if(isset($_POST) & !empty($_POST)){
		...
		if(isset($_FILES) & !empty($_FILES)){
			...
			if(isset($name) && !empty($name)){
				if(($extension == "jpg" || $extension == "jpeg") && $type == "image/jpeg" && $size<=$max_size){
					...
					if(move_uploaded_file($tmp_name, $location.$name)){
						...
						$sql = "INSERT INTO products (product_title, product_desc, catid, shopid, price, image) 
                        VALUES ('$prodname', '$description', '$category', '$shop','$price', '$location$name')";
			...
			$sql = "INSERT INTO products (product_title, product_desc, catid,shopid, price) 
            VALUES ('$prodname', '$description', '$category', '$category','$price')";
			$res = mysqli_query($connection, $sql);
			if($res){
				$smsg= "Product Added!";
				//header('location: products.php');
			  ...
			  <div class="form-group">
			    ...
			    <select class="form-control" id="productshop" name="productshop">
				  ...
				  <?php 	
					...
					$sres = mysqli_query($connection, $shopsql); 
